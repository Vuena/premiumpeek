...
export default function TestingPage() {
  const t = useTranslations("DashboardTesting")
  ...
  const handleScreenshotSelect = (appId: string, file: File | null) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setError(t("screenshotLimit")); setTimeout(() => setError(""), 4000)
      return
    }
    if (!file.type.startsWith("image/")) {
      setError(t("screenshotType")); setTimeout(() => setError(""), 4000)
      return
    }
  ...
  const markTested = async (appId: string, packId: string) => {
    ...
    if (!file) {
      setError(t("screenshotRequired")); setTimeout(() => setError(""), 4000)
      return
    }
    ...
    try {
      ...
      addToast("success", t("testSaved"))
    } catch (err) { console.error("Failed to record test activity:", err)
      addToast("error", t("testSaveError"))
      setError(t("uploadError")); setTimeout(() => setError(""), 4000)
    } finally {
  ...
  const submitComplaint = async (appId: string) => {
    ...
    try {
      ...
      await addComplaint({
        ...
        complainedByName: user.displayName || user.email || "Unnamed",
        targetUid: app.uid,
        targetName: targetMember?.displayName || "Unknown",
        reason: complaintReason,
      })
      setComplaintOpen(null)
      setComplaintReason("")
    } catch (err) { console.error("Failed to submit complaint:", err)
      addToast("error", t("complaintSubmitError"))
    } finally {
      ...
      {appsToTest.length === 0 ? (
        <Card className="border-0 shadow-sm">
          <CardContent className="p-12 text-center">
            ...
            <h2 className="text-lg font-semibold mb-2">{t("noTestsTitle")}</h2>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              {isPremium ? t("noTestsDescPremium") : t("noTestsDesc")}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-zinc-500">
              {t("testedCount", { count: testedToday.length, total: appsToTest.length })}
            </p>
                ...
                <CardContent className="p-5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!tested && (
                        <Button size="sm" onClick={() => markTested(app.id, app.packId || "")} disabled={uploading[app.id]}>
                          {uploading[app.id] ? <Loader2 className="h-4 w-4 animate-spin" /> : t("tested")}
                        </Button>
                      )}
                    </div>
                  </div>
                      ...
                      <div>
                        ...
                        {screenshotPreviews[app.id] ? (
                          <div className="relative inline-block">
                            <img src={screenshotPreviews[app.id]} alt={t("addScreenshot")} className="h-32 rounded-xl object-cover border border-zinc-300 dark:border-zinc-600" />
                            <button
                  ...
                  {complaintOpen === app.id && !tested && (
                    <div className="mt-3 ml-14 p-3 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
                      <p className="text-xs font-medium text-red-700 dark:text-red-400 mb-2">{t("complaintTitle")}</p>
                      <textarea
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="destructive" onClick={() => submitComplaint(app.id)} disabled={complaintSubmitting || !complaintReason.trim()}>
                          {complaintSubmitting ? <Loader2 className="h-3 w-3 animate-spin" /> : t("report")}
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => { setComplaintOpen(null); setComplaintReason("") }}>{t("cancel")}</Button>
                      </div>
                    </div>
                  )}
                </CardContent>
