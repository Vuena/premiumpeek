  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold">{t("welcome")}{user.displayName ? `, ${user.displayName}` : ""}</h1>
          <p className="text-sm text-muted mt-1">{t("subtitle")}</p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/dashboard/apps/new">
            <Button className="gap-2"><Plus size={16} />{t("uploadApp")}</Button>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-8">
        {[
          { icon: Smartphone, label: t("todayTest"), value: testedCount.toString(), href: "/dashboard/testing", color: "text-green-600 bg-green-50 dark:bg-green-950/30" },
          { icon: FileText, label: t("apps"), value: apps.length.toString(), href: "/dashboard/apps", color: "text-purple-600 bg-purple-50 dark:bg-purple-950/30" },
        ].map((stat) => (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            <Card className={`border-2 shadow-md bg-gradient-to-br ${
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  <h2 className="font-semibold text-lg">{t("pack")}</h2>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ml-auto ${
                    ...
                    {currentPack.status === "forming" ? t("statusForming") :
                     currentPack.status === "installing" ? t("statusInstalling") :
                     currentPack.status === "completed" ? t("statusCompleted") : t("statusTesting")}
                  </span>
                </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted">
                        {currentPack.status === "forming"
                          ? t("memberCount", { count: currentPack.members.length })
                          : currentPack.status === "installing"
                          ? t("installingPhase", { count: currentPack.members.length })
                          : currentPack.status === "completed"
                          ? t("completedWith", { count: currentPack.members.length })
                          : t("dayProgress", { current: currentPack.currentDay, total: currentPack.totalDays, count: currentPack.members.length })}
                      </p>
                    </div>
                  </div>
          ...
          {formingPacks.map(p => (
            <Card key={p.id} className="border-2 border-yellow-200 dark:border-yellow-800 shadow-md bg-gradient-to-br from-yellow-50/40 to-white dark:from-yellow-950/10 dark:to-zinc-950">
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  <h2 className="font-semibold text-lg">{t("availablePack")}</h2>
                  <span className="text-[10px] font-medium px-2 py-0.5 rounded-full ml-auto bg-yellow-100 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-400">{t("statusForming")}</span>
                </div>
                <div className="flex items-center justify-between p-4 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted">{t("memberCount", { count: p.members.length })}</p>
                    </div>
                  </div>
          ...
          <Card className="border-cardborder shadow-sm">
            <CardContent className="p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  { icon: Layers, label: t("uploadApp"), href: "/dashboard/apps/new", desc: t("addToPack"), color: "text-purple-600" },
                  { icon: Clock, label: t("todaysTests"), href: "/dashboard/testing", desc: t("doTests"), color: "text-green-600" },
                  { icon: FileText, label: t("reportsAction"), href: "/dashboard/reports", desc: t("viewReports"), color: "text-orange-600" },
                  { icon: Settings, label: t("settingsAction"), href: "/dashboard/settings", desc: t("editProfile"), color: "text-zinc-600" },
                ].map((action) => (
        ...
        <div className="space-y-6">
          <Card className="border-cardborder shadow-sm">
            <CardContent className="p-6">
              <h2 className="font-semibold text-lg mb-4">{t("howItWorks")}</h2>
          ...
          <Card className="border-cardborder shadow-sm">
            <CardContent className="p-6">
              <h2 className="font-semibold text-lg mb-4">{t("statistics")}</h2>
              <div className="space-y-4">
                {[
                  { label: t("yourApps"), value: apps.length.toString() },
                  { label: t("activePack"), value: activePacks.length.toString() },
                ].map(s => (
      ...
      {/* App Selection Modal */}
      {joinPackId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" onClick={() => setJoinPackId(null)} role="dialog" aria-modal="true" aria-label={t("joinPack")}>
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-lg">{t("joinPack")}</h3>
              <button onClick={() => setJoinPackId(null)} aria-label={t("close")} className="cursor-pointer"><X size={20} /></button>
            </div>
            ...
            {apps.filter(a => !a.packId).length > 0 ? (
              <>
                <p className="text-sm text-zinc-500 mb-4">{t("selectApp")}</p>
                  ...
                  {apps.filter(a => !a.packId).map(app => (
                    <div key={app.id} className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
                      <div className="flex items-center gap-3 min-w-0">
                        {app.appIcon ? (
                          <img src={app.appIcon} alt={t("uploadApp")} className="h-9 w-9 rounded-lg object-cover shrink-0" />
                        ) : (
                      ...
                      <Button size="sm" onClick={async () => {
                        ...
                        try {
                          ...
                          setJoinPackId(null)
                          loadData()
                          addToast("success", t("joinSuccess"))
                        } catch (err: any) {
                          setJoinError(err.message || t("joinError"))
                        } finally {
                ...
                <div className="border-t border-zinc-200 dark:border-zinc-700 pt-4">
                  <Link href={`/dashboard/apps/new?packId=${joinPackId}`} onClick={() => setJoinPackId(null)}>
                    <Button variant="outline" className="w-full gap-2"><Plus size={16} /> {t("addNewApp")}</Button>
                  </Link>
                </div>
              </>
            ) : (
              <>
                <p className="text-sm text-zinc-500 mb-4">{t("noAppToJoin")}</p>
                <Link href={`/dashboard/apps/new?packId=${joinPackId}`} onClick={() => setJoinPackId(null)}>
                  <Button className="w-full gap-2"><Plus size={16} /> {t("uploadAppModal")}</Button>
                </Link>
              </>
            )}
          </div>
