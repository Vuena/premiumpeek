...
export default function NewReportPage() {
  ...
  const addItem = () => {
    if (!selectedApp) { setError(t("appRequired")); return }
    setError("")
  ...
  const handleDownload = () => {
    ...
    const dateStr = new Date().toLocaleDateString(locale === "en" ? "en-US" : "tr-TR")
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      addToast("error", t("saveError", { error: err instanceof Error ? err.message : t("unknownError") }))
    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      ...
      <div className="space-y-4">
        ...
        {items.map((item, i) => (
          <div key={i} className="flex items-start gap-2">
            <select value={item.type} onChange={e => updateItem(i, "type", e.target.value)}
              ...
              <option value="uiux">{t("typeUiux")}</option>
              <option value="feature">{t("typeFeature")}</option>
              <option value="other">{t("typeOther")}</option>
            </select>
            <input value={item.description} onChange={e => updateItem(i, "description", e.target.value)}
              placeholder={t("descriptionPlaceholder")} className="flex-1 h-11 rounded-xl border border-zinc-300 dark:border-zinc-600 bg-transparent px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-400" />
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={addItem} className="gap-2"><Plus size={16} /> {t("addItem")}</Button>
        </div>
