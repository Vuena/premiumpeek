...
export default function PackDetailPage() {
  const t = useTranslations("DashboardPacks")
  ...
  useEffect(() => {
    ...
    const tryTransition = async () => {
      ...
      } catch (err) { console.error("Pack status refresh failed:", err) }
    }
    // Check once on mount, then every 5 minutes (CRON handles the main logic)
    tryTransition()
    const interval = setInterval(tryTransition, 300000)
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (remaining <= 0) { setInstallTimeLeft(t("timeExpired")); return }
    ...
    return () => clearInterval(interval)
  }, [pack?.status, pack?.installDeadline, t])
  ...
  const handleInstallScreenshot = (file: File | null) => {
    ...
    if (file.size > 5 * 1024 * 1024) { setInstallError(t("fileSizeError")); setTimeout(() => setInstallError(""), 4000); return }
    if (!file.type.startsWith("image/")) { setInstallError(t("fileTypeError")); setTimeout(() => setInstallError(""), 4000); return }
  ...
  const handleConfirmInstall = async () => {
    ...
    try {
      ...
      if (!storage) { addToast("error", t("storageError")); return }
      ...
      addToast("success", t("installSuccess"))
    } catch (err: any) {
      setInstallError(err.message || t("installError"))
    } finally {
      setActionLoading(false)
    }
  }
  ...
  }, [loading, pack, isMember, router])

  useEffect(() => {
    if (loading || !pack) return
    if (myApps.length === 0 && !isPremium && pack.status === "testing") router.push("/dashboard/apps/new")
  }, [loading, pack, myApps.length, isPremium, router])
  ...
  if (!pack) return <div className="text-center py-16 text-zinc-500">{t("notFound")}</div>
  ...
  const phaseSteps = [
    { label: t("phaseRegister"), status: "forming", icon: Users, done: pack.members.length >= pack.maxMembers },
    { label: t("phaseInstall"), status: "installing", icon: Smartphone, done: pack.status === "testing" || pack.status === "completed" || (pack.status === "installing" && myMemberInfo?.type !== "premium" && pack.members.filter(m => m.type === "free").every(m => m.installConfirmed)) },
    { label: t("phaseTest"), status: "testing", icon: Clock, done: pack.status === "testing" || pack.status === "completed" },
    { label: t("phaseComplete"), status: "completed", icon: Trophy, done: pack.status === "completed" },
  ]
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {isPremium && (
              <span className="text-xs font-medium px-2.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400">{t("premium")}</span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { icon: Users, label: t("statsMembers"), value: `${pack.members.length}/18` },
          { icon: Calendar, label: t("statsStatus"), value: pack.status === "forming" ? t("waiting") : t("days", { current: pack.currentDay || 0, total: pack.totalDays }) },
          { icon: Clock, label: t("statsProgress"), value: pack.status === "forming" ? "-" : `${daysCompleted}/${pack.totalDays}` },
          { icon: CheckCircle2, label: t("statsApps"), value: `${apps.length}` },
        ].map((s) => (
        ...
        <Card className="border-0 shadow-sm mb-8 border-l-4 border-l-blue-500">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-lg">{t("installPhase")}</h2>
            </div>

            <div className="mb-4">
              <p className="text-sm text-muted mb-1">{t("timeRemaining")}</p>
              <p className={`text-2xl font-bold font-mono ${installTimeLeft === t("timeExpired") ? "text-red-600" : "text-blue-600"}`}>
                {installTimeLeft || t("calculating")}
              </p>
            </div>
            ...
            {apps.length === 0 ? (
              <div className="text-center py-6 text-zinc-400">
                <p className="text-sm">{t("noAppsYet")}</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={app.id} className="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{app.appName}{isMyApp ? ` ${t("yours")}` : ""}</p>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                {installScreenshotPreview ? (
                  <div className="relative inline-block">
                    <img src={installScreenshotPreview} alt={t("installPhase")} className="h-24 rounded-xl object-cover border border-zinc-300 dark:border-zinc-600" />
                    <button
      ...
      {pack.status === "installing" && isPremium && (
        <Card className="border-0 shadow-sm mb-8 border-l-4 border-l-amber-500">
          <CardContent className="p-6">
            <h2 className="font-semibold text-lg mb-2">{t("premiumMember")}</h2>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-6">
            <h2 className="font-semibold text-lg mb-4">{t("membersTitle", { count: pack.members.length })}</h2>
                ...
                <div key={member.uid} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {member.type === "premium" && (
                        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400">{t("premium")}</span>
                      )}
                      {member.uid === pack.createdBy && (
                        <span className="text-[10px] text-zinc-500">{t("founder")}</span>
                      )}
                    </div>
                    <p className="text-xs text-zinc-500">
                      {pack.status === "installing" && !member.installConfirmed && member.type === "free" ? t("notInstalled") :
                       pack.status === "installing" && member.installConfirmed ? t("installed") :
                       member.type === "premium" ? t("premium") : t("member")}
                    </p>
                  </div>
                </div>
              ))}
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-lg">{t("appsTitle", { count: apps.length })}</h2>
              {pack.status === "testing" && !isPremium && (
                <Link href="/dashboard/apps/new">
                  <Button size="sm">{t("addApp")}</Button>
                </Link>
              )}
            </div>
            ...
            {apps.length === 0 ? (
              <div className="text-center py-8 text-zinc-400">
                <p className="text-sm">{t("noAppsDesc")}</p>
                <p className="text-xs mt-1">{t("noAppsSub")}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {apps.map((app) => (
