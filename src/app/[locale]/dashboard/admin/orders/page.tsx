  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold">{t("title", { count: orders.length })}</h1>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-zinc-50 dark:bg-zinc-800">
            <tr>
              <th className="text-left px-4 py-3 font-medium">{t("tableApp")}</th>
              <th className="text-left px-4 py-3 font-medium">{t("tableUser")}</th>
              <th className="text-center px-4 py-3 font-medium">{t("tableAmount")}</th>
              <th className="text-center px-4 py-3 font-medium">{t("tableTxHash")}</th>
              <th className="text-center px-4 py-3 font-medium">{t("tableStatus")}</th>
              <th className="text-center px-4 py-3 font-medium">{t("tableActions")}</th>
            </tr>
          </thead>
              ...
              <tr key={o.id} className="border-t border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                ...
                <td className="px-4 py-3 text-center">
                  {o.txHash ? (
                    ...
                    <span className="text-zinc-400">{t("noTxHash")}</span>
                  )}
                </td>
                <td className="px-4 py-3 text-center">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-center gap-1">
                    {o.status === "awaiting_confirmation" && (
                      <Button variant="ghost" size="sm" onClick={() => updateStatus(o.id, "paid")} title={t("confirmPaymentTitle")}>
                        <CreditCard size={14} className="text-blue-600" />
                      </Button>
                    )}
                    {o.status === "paid" && (
                      <Button variant="ghost" size="sm" onClick={() => handleAssignTesters(o.id)} title={t("assignTestersTitle")}>
                        <Users size={14} className="text-green-600" />
                      </Button>
                    )}
                    {o.status === "testing" && (
                      <Button variant="ghost" size="sm" onClick={() => updateStatus(o.id, "completed")} title={t("completeTitle")}>
                        <CheckCircle2 size={14} className="text-green-600" />
                      </Button>
                    )}
                    {o.status !== "refunded" && o.status !== "completed" && (
                      <Button variant="ghost" size="sm" onClick={() => { if (confirm(t("refundConfirm"))) updateStatus(o.id, "refunded") }} title={t("refundTitle")}>
            ...
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                <div className="text-zinc-500">
                  <span className="text-xs">{t("mobileLabelUser")}</span>
                  <p className="font-medium text-xs truncate">{o.userEmail || o.uid?.slice(0, 8)}</p>
                </div>
                <div className="text-zinc-500">
                  <span className="text-xs">{t("mobileLabelAmount")}</span>
                  <p className="font-medium">${o.amount} {o.currency}</p>
                </div>
              </div>
              {o.txHash && (
                <div className="mb-3">
                  <span className="text-xs text-zinc-500">{t("mobileLabelTx")}</span>
                  <a href={`https://tronscan.org/#/transaction/${o.txHash}`} target="_blank" rel="noopener noreferrer"
