...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({ type: "error", text: data.error || "Bir hata oluştu" })
      }
    } catch {
      setStatus({ type: "error", text: "Bir hata oluştu. Lütfen tekrar dene." })
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-16">
      ...
      <p className="text-muted mb-12">Soruların mı var? Bize ulaş.</p>
      ...
      <div className="rounded-2xl bg-zinc-100 dark:bg-[#121212] p-8">
        <h2 className="text-xl font-bold mb-6">Mesaj Gönder</h2>
        {status && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5">Adın</label>
              <input value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                className="flex h-11 w-full rounded-xl border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-400" placeholder="Adın" required />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">E-posta</label>
              <input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})}
                className="flex h-11 w-full rounded-xl border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-400" placeholder="ornek@email.com" required />
            </div>
          </div>
          <div>
