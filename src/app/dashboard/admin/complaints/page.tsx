    ...
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {pendingComplaints.length > 0 ? `${pendingComplaints.length} bekleyen şikayet` : "Bekleyen şikayet yok"}
          </p>
        </div>
      </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted mb-1">
                      <strong>Şikayet eden:</strong> {complaint.complainedByName}
                    </p>
                    <p className="text-xs text-muted mb-1">
                      <strong>Şikayet edilen:</strong> {complaint.targetName} (UID: {complaint.targetUid.slice(0, 8)}...)
                    </p>
                    <p className="text-xs text-muted mb-1">
                      <strong>Pack ID:</strong> {complaint.packId}
                    </p>
                    <p className="text-xs bg-zinc-50 dark:bg-zinc-800 p-2 rounded-lg mt-2">
                      <strong>Sebep:</strong> {complaint.reason}
                    </p>
                  </div>
              ...
              <Card key={complaint.id} className="border-0 shadow-sm opacity-60">
                <CardContent className="p-4 flex items-center justify-between">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full shrink-0 ${
                    ...
                    {complaint.status === "resolved" ? "Atıldı" : "Reddedildi"}
                  </span>
                </CardContent>
              </Card>
            ))}
      ...
      {complaints.length === 0 && (
        <div className="text-center py-16 text-zinc-400">
          <p className="text-sm">Henüz hiç şikayet yok</p>
        </div>
      )}
    </div>
  )
